...
- (void)viewDidLoad {
    ...
    [self setNavigationBarTitle:@"我的钱包" leftImage:[UIImage imageNamed:@"ic_stat_back_02_s"] andRightImage:nil];
...
- (void)walletData {
    ...
    [hm postRequetInterfaceData:nil withInterfaceName:@"user/useraccount"];
...
//                                     };
//
//    [hm postRequetInterfaceData:dataDic withInterfaceName:@"user/useraccountlogs"];
...
//                saio = [MoneyBagRecord mj_objecta:rd.data];
    ...
    [hm getRequetInterfaceData:dataDic withInterfaceName:@"api/frontend.user/money"];
}
...
//
        cell.titleLabel.text = @"充值";//so.typeStr;
        cell.dateTimeLabel.text = @"2019-09-24 17:45";//[Util time_timestampToString:[so.iitTime doubleValue]];
//
//
//        if ([so.amount intValue]>0) {
        cell.moneyLabel.text = @"+100";//[NSString stringWithFormat:@"+%.2f", [so.amount doubleValue]];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    sectionTitleLabel.text = @"本月";//[sectionArr objectAtIndex:section-1];
