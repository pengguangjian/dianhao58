...
- (void)setUpProperties
{
    ...
    [preBtn setTitle:NSLocalizedString(@"yulan", nil) forState:UIControlStateNormal];///
...
- (void)startPhotoAssetsViewCell:(JKAssetsViewCell *)assetsCell
{
    if (self.selectedAssetArray.count>=self.maximumNumberOfSelection) {
        ...
        str = NSLocalizedString(@"zuiduoxuanzztupian", nil);
...
- (void)resetFinishFrame
{
    ...
    self.finishLabel.text = [NSString stringWithFormat:@"%@(%ld)",NSLocalizedString(@"xiayibu", nil),self.selectedAssetArray.count];
...
- (UIToolbar *)toolbar{
    if (!_toolbar) {
        _toolbar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, self.view.height-44-SafeAreaTopHeight, self.view.width, 44)];
        ...
        label.text = NSLocalizedString(@"yuantu", nil);
        [label sizeToFit];
        ...
        _finishLabel.text = [NSString stringWithFormat:@"%@(5)",NSLocalizedString(@"xiayibu", nil)];
